  ...
  <Card :dis-hover="true" :shadow="false" class="m-t-15">
    ...
    <div style="text-align: center">
      ...
      <template v-if="currentStep == 0">
        ...
        >
          {{ __('下载') }}
          <Link :to="doc.download_path" target="_blank">
            《{{ __('批量新增%s模板', doc.title) }}》
          </Link>
          ,{{ __('录入更新数据后上传') }}
        </div>
...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      this.apiPost('job:job', {
        name: this.file.name,
        content: JSON.stringify(this.importData),
        ...
        this.apiPatch('job:job', res.id, 'progress').then(
          (jobRes) => {
            this.fetchJob(res.id)
          },
          () => {},
    ...
    prepareDoc(doc) {
      doc.cancelled_path = `/${doc.type}-batch-import-list`
      doc.next_path = `/${doc.type}-batch-import-create`
      doc.back_list_path = `/${doc.type}`
      doc.download_path = `/import/${doc.type}.csv`

      this.menuLeft.push({
        title: __('导入') + doc.title,
        icon: 'md-list',
        path: doc.cancelled_path,
      })
    },
  },
  unmounted() {
