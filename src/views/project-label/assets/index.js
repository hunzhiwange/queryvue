...
export default {
                            ...
                            <div>
                                <buttonGroup size="small" shape="circle">
                                    <i-button
                                        ...
                                        v-show={utils.permission('project_label_edit_button')}>
                                        {this.__('编辑')}
                                    </i-button>
                                    <i-button
                                        type="text"
                                        onClick={() => this.remove(params)}
                                        v-show={utils.permission('project_label_delete_button')}>
                                        {this.__('删除')}
                                    </i-button>
                                </buttonGroup>
                            </div>
    ...
    methods: {
        ...
        remove(params) {
            this.$Modal.confirm({
                title: this.__('提示'),
                content: this.__('确认删除该项目方法?'),
                onOk: () => {
                    this.loadingTable = !this.loadingTable
                    this.apiDelete('project-label', params.row.id).then(res => {
        ...
        statusMany(type) {
            ...
            this.apiPost('project-label/status', data).then(res => {
        ...
        saveProjectLabel(form) {
            ...
            this.apiPost('project-label', formData).then(
                res => {
                    this.refresh()
                    this.loading = !this.loading
        ...
        updateProjectLabel(form) {
            ...
            this.apiPut('project-label', this.formItem.id, formData).then(
                res => {
                    this.refresh()
                    this.loading = !this.loading
                    this.cancelMinForm(form)
